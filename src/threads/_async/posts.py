...
class AsyncPostsClient(BasePostsClient):
    """Asynchronous client for post operations."""
    ...
    async def create_and_publish(
        ...
        *,
        ...
        container = await self._parent.media.create_container(
            user_id,
            media_type,
            text=text,
            image_url=image_url,
    ...
    async def _wait_for_container(
        ...
        while elapsed < max_wait_time:
            status = await self._parent.media.get_container_status(container_id)
        ...
        raise ContainerError(
            f"Container not ready after {max_wait_time}s (status: {status.status})"
        )
    ...
    async def get_publishing_limit(self, user_id: str) -> PublishingLimit:
